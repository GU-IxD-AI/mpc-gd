...
enum ChooserType{
    case exactlyOne, oneOrNone, multiple, none
}
...
class HKComponentCardChooser: HKComponent, HKAnimatedComponent{
    ...
    internal var unselectedComponents: [HKComponent] = []
    
    internal var selectedComponents: [HKComponent] = []
    ...
    init(cardIDs: [String], size: CGSize, cardSize: CGSize, separatorWidth: CGFloat, separatorAtStart: Bool, unselectedComponents: [HKComponent], selectedComponents: [HKComponent], chooserType: ChooserType, verticalBarImage: UIImage, scrollBarImage: UIImage, scrollBarType: ScrollBarType, timeBetweenTaps: TimeInterval = TimeInterval(0.5), useCropNode: Bool = true, snapToCentre: Bool = false){
        ...
        self.unselectedComponents = unselectedComponents
        numCards = unselectedComponents.count
        ...
        for pos in 0..<numCards{
            ...
            let unselectedComponent = unselectedComponents[pos]
            unselectedComponent.position = CGPoint(x: xPos, y: 0)
            trayWidth = trayWidth + cardSize.width + separatorWidth
            let selectedComponent = selectedComponents[pos]
            selectedComponent.position = CGPoint(x: xPos, y: 0)
            trayNode.addChild(selectedComponent)
        ...
        // HACK to constrain touchable drag region to original UI dimensions
        trayNodeDragRect = CGRect(origin: CGPoint.zero, size: size)
        trayNodeDragRect.origin.x -= trayNodeDragRect.size.width / 2.0
        trayNodeDragRect.origin.y -= trayNodeDragRect.size.height / 2.0
    ...
    func updateAfterValuesChanged() {
        ...
        for pos in 0..<numCards{
            if isSelected[pos]{
                firstSelectedX = selectedComponents[pos].position.x
                break
            }
        }
        ...
        if touches.count == 1{
            ...
            if let touch = touches.first {
                if touch.tapCount <= 1{
                    ...
                    if trayNodeDragRect.contains(touchDownPoint){
        ...
        if touches.count == 1{
            if let touch = touches.first {
                if touch.tapCount <= 1 && acknowledgeDrag{
                    let timeSinceLastMove = timeOfLastMove != nil ? abs(timeOfLastMove.timeIntervalSinceNow) : 0.0
                    ...
                    if timeOfTouchesStart != nil{
                        let timeSinceTouchesStart = timeOfTouchesStart != nil ? abs(timeOfTouchesStart.timeIntervalSinceNow) : 0.0
    ...
    fileprivate func setCardVisibilities(){
        for pos in 0..<numCards{
            let selectedImage = selectedComponents[pos]
            let unselectedImage = unselectedComponents[pos]
            let x = selectedImage.position.x + trayNode.position.x - cardSize.width/2
            let rect = CGRect(x: x, y: -10, width: cardSize.width, height: 20)
    ...
    fileprivate func updateCardAlphas(){
        for pos in 0..<numCards{
            if isSelected[pos]{
                selectedComponents[pos].alpha = 1
                unselectedComponents[pos].alpha = 0
            }
            else{
                selectedComponents[pos].alpha = 0
                unselectedComponents[pos].alpha = 1
            }
        }
    }
    ...
    fileprivate func startTrayMovement(_ dx: CGFloat){
        ...
        if abs(totalMovement) >= 1{
            for pos in 0..<numCards{
                selectedComponents[pos].isHidden = false
    ...
    fileprivate func handleTap(){
        if !isAlreadyTapping{
            ...
            for pos in 0..<numCards{
                if unselectedComponents[pos].contains(point) {
                    tapPosition = pos
                }
            }
            if tapPosition != nil{
        ...
        if chooserType == .exactlyOne || chooserType == .oneOrNone{
            if !isSelected[pos]{
                ...
                if chooserType == .exactlyOne{
                    selectedComponents[pos].run(scaleAction, completion: {
                        if self.tapCode != nil{
                            self.tapCode()
                        }
    ...
    fileprivate func toggleImageSelection(_ imagePos: Int){
        ...
        if !isSelected[imagePos]{
            unselectedComponents[imagePos].run(scaleAction, completion: completionBlock)
            selectedComponents[imagePos].alpha = 0
            selectedComponents[imagePos].run(scaleAction)
            selectedComponents[imagePos].run(fadeIn)
            if chooserType == .exactlyOne {
                for pos in 0..<numCards{
                    if isSelected[pos]{
                        selectedComponents[pos].alpha = 0
            ...
            selectedComponents[imagePos].run(scaleAction, completion: completionBlock)
        }
    }
    ...
    func hideComponents(){
        for comp in unselectedComponents{
            var f = comp.frame
